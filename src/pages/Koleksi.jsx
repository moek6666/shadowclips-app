...
export default function Koleksi({ supabase }) {
    ...
    const fetchCollections = async () => {
        ...
        const { data, error } = await supabase.from('videos').select('labels, img').order('created_at', { ascending: false });
        ...
        (data || []).forEach(video => {
            ...
            if (cleanLabel) {
                ...
                if (existingKey) { grouped[existingKey].count += 1; }
                else {
                    const displayLabel = cleanLabel.replace(/\b\w/g, c => c.toUpperCase());
                    grouped[displayLabel] = { name: displayLabel, count: 1, coverImage: getImageUrl(video.img) };
                }
            }
        });
    ...
    const filteredCollections = collections.filter(c => c.name.toLowerCase().includes(searchInput.toLowerCase()));
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                        {isLoading ? (
                            Array.from({ length: 8 }).map((_, i) => (
                                <div key={i} className="aspect-[4/3] bg-zinc-900 border-none rounded-[4px] animate-pulse"></div>
                            ))
                        ) : filteredCollections.length > 0 ? (
                            filteredCollections.map((col, index) => (
                                <div key={index} onClick={() => { window.location.href = `/koleksi/${encodeURIComponent(col.name)}`; }}
                                ...
                                >
                                    <img src={col.coverImage || '/placeholder-image.jpg'} alt={col.name} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" />
                                    ...
                                    <div className="absolute inset-0 p-6 flex flex-col justify-end">
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="bg-[#106EBE] text-white px-2.5 py-1 rounded-[3px] text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 shadow-[0_0_10px_rgba(16,110,190,0.4)] border-none">
                                                <FolderOpen className="w-3 h-3" /> {col.count} Videos
                                            </span>
                                        </div>
                                        <h3 className="text-2xl font-bold text-zinc-300 group-hover:text-white transition-colors drop-shadow-md">
                                            {col.name}
                                        </h3>
                                    </div>
                                </div>
