...
export default function Streaming({ supabase }) {
    ...
    }, [activeServer, isPlaying, isOriginalOnline]);
    ...
    const checkVipAccess = useCallback(async (videoId, vidData, forceCommented = null, forceLiked = null) => {
        ...
        if (isPaidContent) {
            if (isUserPremium) {
                setIsVipUnlocked(true); setLockReason('none'); setSecureUrls({ original: uOriginal, main: uMain, alt: uAlt, alt2: uAlt2, img: uImg });
            } else {
                setIsVipUnlocked(false); setLockReason('payment'); setSecureUrls({ original: '', main: '', alt: '', alt2: '', img: uImg });
            }
        } else if (isExclusiveContent) {
            if (isUserPremium || (isLiked && isCommented)) {
                setIsVipUnlocked(true); setLockReason('none'); setSecureUrls({ original: uOriginal, main: uMain, alt: uAlt, alt2: uAlt2, img: uImg });
            } else {
                setIsVipUnlocked(false); setLockReason('exclusive'); setSecureUrls({ original: '', main: '', alt: '', alt2: '', img: uImg });
            }
        } else {
            setIsVipUnlocked(true); setLockReason('none'); setSecureUrls({ original: uOriginal, main: uMain, alt: uAlt, alt2: uAlt2, img: uImg });
        }
    ...
    const hasOriginal = secureUrls.original && secureUrls.original !== 'EMPTY' && String(secureUrls.original).trim() !== '';
    ...
    let currentVideoUrl = '';
    if (effectiveServer === 'original' && hasOriginal) currentVideoUrl = secureUrls.original;
                            ...
                            {!isVipUnlocked ? (
                                lockReason === 'payment' ? (
                                    <div className="absolute inset-0 flex flex-col items-center justify-center p-4 sm:p-8 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-3xl z-50 text-center transition-colors overflow-y-auto border-none">
                                        ...
                                        <div className="w-12 h-12 sm:w-20 sm:h-20 bg-amber-500 rounded-[10px] flex items-center justify-center shadow-md mb-3 transform rotate-3 shrink-0 border-none">
                                            <Lock className="w-6 h-6 sm:w-10 sm:h-10 text-white border-none" />
                                        </div>
                                        <h2 className="text-lg sm:text-3xl font-black text-zinc-900 dark:text-white mb-1.5 border-none">Premium Content</h2>
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center p-4 sm:p-8 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-3xl z-50 text-center transition-colors border-none">
                                        <div className="absolute inset-0 z-[-1] opacity-10 pointer-events-none border-none">
                                            <img src={coverImage} className="w-full h-full object-cover blur-sm border-none" alt="locked background" />
                                        </div>
                                        <div className="w-12 h-12 sm:w-20 sm:h-20 bg-gradient-to-br from-[#106EBE] to-[#0e5c9f] rounded-[10px] flex items-center justify-center shadow-md mb-3 transform rotate-3 shrink-0 border-none">
                                            <Lock className="w-6 h-6 sm:w-10 h-10 text-white border-none" />
                                        </div>
                                        <h2 className="text-lg sm:text-3xl font-black text-zinc-900 dark:text-white mb-1.5 border-none">VIP Content Locked</h2>
                                ...
                                <div className="w-full h-full overflow-y-auto p-4 custom-scrollbar bg-zinc-100 dark:bg-zinc-950/40 border-none" onContextMenu={(e) => e.preventDefault()}>
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 border-none">
                                        {galleryImages.map((imgUrl, idx) => (
                                            <div key={idx} onClick={() => setSelectedImage(imgUrl)} className="relative group cursor-pointer aspect-[3/4] rounded-2xl overflow-hidden bg-zinc-200 dark:bg-zinc-900 border-none shadow-sm hover:shadow-md transition-all">
                                                <img src={imgUrl} alt={`Gallery ${idx}`} className="w-full h-full object-cover group-hover:scale-110 border-none" loading="lazy" />
                                                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 flex items-center justify-center border-none">
                                                    <div className="bg-white/10 p-3 rounded-[10px] scale-50 group-hover:scale-100 border-none"><ZoomIn className="w-6 h-6 text-white border-none" /></div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="relative min-w-0 border-none">
                                    {serverOptions.length > 1 ? (
                                        <>
                                            ...
                                            <div className={`absolute top-full right-0 sm:left-0 sm:right-auto mt-2 w-56 bg-white dark:bg-zinc-900/95 backdrop-blur-xl rounded-xl shadow-xl border-none overflow-hidden z-40 flex flex-col py-1.5 transition-all origin-top ${isServerDropdownOpen ? 'opacity-100 scale-y-100 visible' : 'opacity-0 scale-y-95 invisible'}`}>
