...
export default function ModalLoginWrapper(props) {
    // Kita hapus GoogleOAuthProvider karena auth Google dialihkan via Supabase OAuth agar kompatibel dengan APK Android
...
function ModalLogin({ isOpen, onClose, supabase }) {
    ...
    // UNIVERSAL OAUTH LOGIN (Google & Discord) - Aman untuk Web & APK Android
    const handleSocialLogin = async (provider) => {
        ...
        try {
            // Tentukan URL redirect otomatis: jika di Android pakai custom scheme, jika di web pakai origin saat ini
            const isNative = Capacitor.isNativePlatform();
            const redirectUrl = isNative
                ? 'com.shadowclips.app://profile'
            ...
            setErrorMsg(`Gagal terhubung ke ${provider}: ` + err.message);
            setLoading(false);
        }
    };
                        ...
                        {!isForgotPass && (
                            <>
                                ...
                                <div className="grid grid-cols-2 gap-3 w-full mb-4">
                                    {/* GOOGLE LOGIN (Dialihkan via Supabase OAuth untuk kompatibilitas APK) */}
                                    <button
                                        type="button"
