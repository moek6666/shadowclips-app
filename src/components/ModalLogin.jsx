...
import { X, Mail, Lock, Loader2, Eye, EyeOff, User, MonitorPlay, Zap, Radio, Play } from 'lucide-react';
...
function ModalLogin({ isOpen, onClose, supabase }) {
    ...
    const handleForgotPassword = async () => {
        if (!email) {
            setErrorMsg('Masukkan email Anda di kolom atas untuk mereset password.');
            setSuccessMsg('');
            return;
        }
        ...
        try {
            const { error } = await supabase.auth.resetPasswordForEmail(email, {
                // Nantinya Bos perlu membuat halaman /update-password untuk mengganti password baru
            ...
            >
                ...
                <div className="w-full md:w-[45%] p-8 sm:p-12 flex flex-col justify-center relative bg-white dark:bg-[#0E1116] border-none transition-colors">
                    ...
                    <div className="w-full max-w-[340px] mx-auto flex flex-col border-none">
                        ...
                        {/* Menampilkan Pesan Error ATAU Pesan Sukses (Lupa Password) */}
                        {errorMsg && (
                        ...
                        <form onSubmit={handleEmailAuth} className="flex flex-col gap-4 border-none">
                            ...
                            <div className="flex flex-col gap-1.5 border-none">
                                ...
                                <div className="relative group border-none">
                                    ...
                                    <input
                                        ...
                                        required={isLogin ? true : false} // Bisa dilepas required-nya untuk forgot password jika perlu
                                        minLength={6}
                            ...
                            {isLogin && (
                                <div className="flex justify-end -mt-1 mb-1 border-none">
                                    <button
                                        ...
                                        disabled={loading}
                                        className="text-[12px] text-[#3b82f6] hover:text-[#60a5fa] hover:underline font-medium bg-transparent cursor-pointer transition-colors border-none disabled:opacity-50"
                                    >
                                        Forgot password?
                                    </button>
                                </div>
                            ...
                            >
                                {loading ? <Loader2 className="w-5 h-5 animate-spin border-none" /> : (isLogin ? 'Login' : 'Sign Up')}
                            </button>
                        </form>
                        ...
                        {/* ========================================== */}
                        {/* 3. TOMBOL GOOGLE & DISCORD BERDAMPINGAN    */}
                        {/* ========================================== */}
                        ...
                        <p className="text-center text-[13px] text-slate-500 dark:text-zinc-400 mt-2 border-none transition-colors">
                            ...
                            <button
                                ...
                                onClick={() => {
                                    setIsLogin(!isLogin);
                                    setErrorMsg('');
                                    setSuccessMsg(''); // Bersihkan pesan saat ganti mode
                                    if (turnstileRef.current) turnstileRef.current.reset();
                                    setCaptchaToken(null);
                                }}
